...
std::vector<std::string> lines(std::string text) {
    ...
    for(int i = 0; i < text.size(); i++){
        if(text[i] == '\n'){
            radek = text.substr(pocatek,i-pocatek);
            vysledek.push_back(radek);
            pocatek = i + 1;
        }
    }
    radek = text.substr(pocatek);
    vysledek.push_back(radek);
    return vysledek;
}
...
std::vector<std::string> words(std::string text) {
    ...
    for(int i = 0; i < text.size(); i++){
        if(text[i] == '\n' || text[i] == ' '){
            slovo = text.substr(pocatek,i-pocatek);
            if(i-pocatek > 0){
                vysledek.push_back(slovo);
            }
            pocatek = i + 1;
        }
    }
    slovo = text.substr(pocatek);
    vysledek.push_back(slovo);
    return vysledek;
}
